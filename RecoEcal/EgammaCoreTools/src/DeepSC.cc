...
namespace reco {
  ...
  DeepSC::DeepSC() {
    session_NN_Barrel_ = tensorflow::createSession(tensorflow::loadGraphDef(
        edm::FileInPath("RecoEcal/EgammaCoreTools/data/deepSC_model_v17_joindet_elegamma_EBEE.pb").fullPath().c_str()));
    session_NN_Endcap_ = tensorflow::createSession(tensorflow::loadGraphDef(
        edm::FileInPath("RecoEcal/EgammaCoreTools/data/deepSC_model_v17_joindet_elegamma_EBEE.pb").fullPath().c_str()));
    x_mean_Barrel_ = std::vector<double>(
        {6.84241156e-03, 1.62242679e-03, 5.81495577e+01, 2.57215845e+01, -7.09402501e-04, -1.27142875e-04,
         1.30375508e+00, 5.67249500e-01, 1.00772582e+00, 1.35803461e-02, -4.29317013e-06, 1.71072024e-02,
         4.90466869e-01, 5.10511982e+00, 8.82101138e-03, 1.04095965e-02, 1.92096066e+00,  1.31476120e-02,
         1.62948213e-05, 1.42948806e-02, 5.92920497e-01, 1.49597644e+00, 3.36213188e-03,  3.06446267e-03});
    x_std_Barrel_ = std::vector<double>({1.31333380e+00, 5.06988411e-01, 9.21157365e+01, 2.98580765e+01, 1.10279784e-01,
                                         3.30488055e-01, 2.62605247e+00, 1.16284769e+00, 1.17047757e-01, 1.11969442e-02,
                                         1.86572967e-04, 1.31036359e-02, 4.01511744e-01, 5.67007350e+00, 6.14304203e-03,
                                         7.24808860e-03, 7.81094814e+00, 1.70392176e-02, 3.05995567e-04, 1.80176053e-02,
                                         1.99316624e+00, 1.88845046e+00, 4.12315715e-03, 4.79639033e-03});
    x_mean_Endcap_ = std::vector<double>(
        {6.84241156e-03, 1.62242679e-03, 5.81495577e+01, 2.57215845e+01, -7.09402501e-04, -1.27142875e-04,
         1.30375508e+00, 5.67249500e-01, 1.00772582e+00, 1.35803461e-02, -4.29317013e-06, 1.71072024e-02,
         4.90466869e-01, 5.10511982e+00, 8.82101138e-03, 1.04095965e-02, 1.92096066e+00,  1.31476120e-02,
         1.62948213e-05, 1.42948806e-02, 5.92920497e-01, 1.49597644e+00, 3.36213188e-03,  3.06446267e-03});
    x_std_Endcap_ = std::vector<double>({1.31333380e+00, 5.06988411e-01, 9.21157365e+01, 2.98580765e+01, 1.10279784e-01,
                                         3.30488055e-01, 2.62605247e+00, 1.16284769e+00, 1.17047757e-01, 1.11969442e-02,
                                         1.86572967e-04, 1.31036359e-02, 4.01511744e-01, 5.67007350e+00, 6.14304203e-03,
                                         7.24808860e-03, 7.81094814e+00, 1.70392176e-02, 3.05995567e-04, 1.80176053e-02,
                                         1.99316624e+00, 1.88845046e+00, 4.12315715e-03, 4.79639033e-03});
  ...
  std::pair<double, double> DeepSC::ComputeCovariances(const CaloCluster cluster,
  ...
  void DeepSC::ComputeVariables(const CaloCluster* seed,
    ...
    double zSide = 0;
    if (PFLayer::fromCaloID(seed->caloID()) == PFLayer::ECAL_ENDCAP && seed->eta() < 0.)
      zSide = -1.;
    if (PFLayer::fromCaloID(seed->caloID()) == PFLayer::ECAL_ENDCAP && seed->eta() > 0.)
      zSide = +1.;
  ...
  float DeepSC::EvaluateNN(const CaloCluster* seed) {
    ...
    if (PFLayer::fromCaloID(seed->caloID()) == PFLayer::ECAL_BARREL) {
      tensorflow::run(
          session_NN_Barrel_, {{NNinput_string_Barrel_.c_str(), NNinput}}, {NNoutput_string_Barrel_.c_str()}, &outputs);
    } else if (PFLayer::fromCaloID(seed->caloID()) == PFLayer::ECAL_ENDCAP) {
      tensorflow::run(
          session_NN_Endcap_, {{NNinput_string_Endcap_.c_str(), NNinput}}, {NNoutput_string_Endcap_.c_str()}, &outputs);
    }
    float NNscore = outputs[0].matrix<float>()(0, 0);
    return NNscore;
  }

  bool DeepSC::InSuperCluster(const CaloCluster* seed,
                              const CaloCluster* cluster,
                              const CaloTopology* topology,
                              const CaloSubdetectorGeometry* ebGeom,
                              const CaloSubdetectorGeometry* eeGeom,
    ...
    if (PFLayer::fromCaloID(seed->caloID()) == PFLayer::ECAL_BARREL) {
      etaWindow = etaWindows_Barrel_[0] + etaWindows_Barrel_[1] * fabs(seedEta) +
                  etaWindows_Barrel_[2] * fabs(seedEta) * fabs(seedEta);
      phiWindow = phiWindows_Barrel_[0] + phiWindows_Barrel_[1] * fabs(seedEta) +
                  phiWindows_Barrel_[2] * fabs(seedEta) * fabs(seedEta);
      if (deta < etaWindow && dphi < phiWindow)
        isIn = true;

      if (seedEt > 0. && seedEt <= 10. && score > NNWPs_Barrel_[0])
        pass = true;
      else if (seedEt > 10. && seedEt <= 20. && score > NNWPs_Barrel_[1])
        pass = true;
      else if (seedEt > 20. && seedEt <= 30. && score > NNWPs_Barrel_[2])
        pass = true;
      else if (seedEt > 30. && seedEt <= 40. && score > NNWPs_Barrel_[3])
        pass = true;
      else if (seedEt > 40. && seedEt <= 50. && score > NNWPs_Barrel_[4])
        pass = true;
      else if (seedEt > 50. && seedEt <= 60. && score > NNWPs_Barrel_[5])
        pass = true;
      else if (seedEt > 60. && seedEt <= 70. && score > NNWPs_Barrel_[6])
        pass = true;
      else if (seedEt > 70. && seedEt <= 80. && score > NNWPs_Barrel_[7])
        pass = true;
      else if (seedEt > 80. && seedEt <= 90. && score > NNWPs_Barrel_[8])
        pass = true;
      else if (seedEt > 90. && score > NNWPs_Barrel_[9])
        pass = true;

    } else if (PFLayer::fromCaloID(seed->caloID()) == PFLayer::ECAL_ENDCAP) {
      etaWindow = etaWindows_Endcap_[0] + etaWindows_Endcap_[1] * fabs(seedEta) +
                  etaWindows_Endcap_[2] * fabs(seedEta) * fabs(seedEta);
      phiWindow = phiWindows_Endcap_[0] + phiWindows_Endcap_[1] * fabs(seedEta) +
                  phiWindows_Endcap_[2] * fabs(seedEta) * fabs(seedEta);
      if (deta < etaWindow && dphi < phiWindow)
        isIn = true;

      if (seedEt > 0. && seedEt <= 10. && score > NNWPs_Endcap_[0])
        pass = true;
      else if (seedEt > 10. && seedEt <= 20. && score > NNWPs_Endcap_[1])
        pass = true;
      else if (seedEt > 20. && seedEt <= 30. && score > NNWPs_Endcap_[2])
        pass = true;
      else if (seedEt > 30. && seedEt <= 40. && score > NNWPs_Endcap_[3])
        pass = true;
      else if (seedEt > 40. && seedEt <= 50. && score > NNWPs_Endcap_[4])
        pass = true;
      else if (seedEt > 50. && seedEt <= 60. && score > NNWPs_Endcap_[5])
        pass = true;
      else if (seedEt > 60. && seedEt <= 70. && score > NNWPs_Endcap_[6])
        pass = true;
      else if (seedEt > 70. && seedEt <= 80. && score > NNWPs_Endcap_[7])
        pass = true;
      else if (seedEt > 80. && seedEt <= 90. && score > NNWPs_Endcap_[8])
        pass = true;
      else if (seedEt > 90. && score > NNWPs_Endcap_[9])
        pass = true;
    }
    return (isIn && pass);
  }

  void DeepSC::SetXtalsInWindow(DetId seedDetId,
    ...
    for (unsigned int iXtal = 0; iXtal < v_id.size(); iXtal++) {
      ...
      if (fabs(eta_xtal - eta_seed) < etawidthSuperCluster &&
          fabs(TVector2::Phi_mpi_pi(phi_xtal - phi_seed)) < phiwidthSuperCluster &&
          seedDetId.subdetId() == v_id.at(iXtal).subdetId())
        xtals_inWindow_.push_back(v_id.at(iXtal));
    }
  }

  std::pair<double, double> DeepSC::GetMaximumDetaDphi(DetId* seedDetId,
    ...
    for (unsigned int iXtal = 0; iXtal < idMatrix->size(); iXtal++) {
      ...
      if (fabs(eta_xtal - eta_seed) > maxDeta)
        maxDeta = fabs(eta_xtal - eta_seed);
      if (fabs(TVector2::Phi_mpi_pi(phi_xtal - phi_seed)) > maxDphi)
        maxDphi = fabs(TVector2::Phi_mpi_pi(phi_xtal - phi_seed));
    }
  ...
  void DeepSC::DeepSCID(const reco::SuperCluster& sc,
                        ...
                        const EcalRecHitCollection* recHitsEE) {
    DeepSCID(
        sc.clustersBegin(), sc.clustersEnd(), nclusters, EoutsideDeepSC, topology, ebGeom, eeGeom, recHitsEB, recHitsEE);
  }
  ...
  void DeepSC::DeepSCID(const CaloClusterPtrVector& clusters,
                        ...
                        const EcalRecHitCollection* recHitsEE) {
    DeepSCID(
        clusters.begin(), clusters.end(), nclusters, EoutsideDeepSC, topology, ebGeom, eeGeom, recHitsEB, recHitsEE);
  }
  ...
  void DeepSC::DeepSCID(const std::vector<const CaloCluster*>& clusters,
                        ...
                        const EcalRecHitCollection* recHitsEE) {
    DeepSCID(
        clusters.cbegin(), clusters.cend(), nclusters, EoutsideDeepSC, topology, ebGeom, eeGeom, recHitsEB, recHitsEE);
  }
  ...
  void DeepSC::FillDeepSCVar(const std::vector<CaloCluster>& clusters,
                             const CaloTopology* topology,
                             const CaloSubdetectorGeometry* ebGeom,
                             const CaloSubdetectorGeometry* eeGeom,
                             const EcalRecHitCollection* recHitsEB,
    ...
    included_ = insideDeep.size();
    excluded_ = outsideDeep.size();
    ...
    for (unsigned int i = 0; i < outsideDeep.size(); ++i) {
      ...
      Et_Outside_DeepSC_ = clusters[index].energy() * sin(clusters[index].position().theta()) + Et_Outside_DeepSC_;
    }
