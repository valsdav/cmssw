  ...
  : cfg_(cfg)
{
  ...
  clsX_ = tensorflow::Tensor(tensorflow::DT_FLOAT, {cfg_.batchSize, cfg_.maxNClusters, cfg_.nClusterFeatures});
  windX_ = tensorflow::Tensor(tensorflow::DT_FLOAT, {cfg_.batchSize, cfg_.nWindowFeatures});
  hitsX_ = tensorflow::Tensor(tensorflow::DT_FLOAT, {cfg_.batchSize, cfg_.maxNClusters, cfg_.maxNRechits, cfg_.nRechitsFeatures});
  isSeedX_ = tensorflow::Tensor(tensorflow::DT_FLOAT, {cfg_.batchSize, cfg_.maxNClusters, 1});
  nClsSize_ = tensorflow::Tensor(tensorflow::DT_FLOAT, {cfg_.batchSize});
}
...
std::vector<std::vector<float>> DeepSCGraphEvaluation::evaluate(const DeepSCInputs& inputs) const {
  ...
  */
  ...
  size_t totInputs = inputs.clustersX.size(); 
  size_t nBatches = totInputs / cfg_.batchSize;
  if (totInputs % cfg_.batchSize > 0) nBatches++;

  for (size_t b_ = 0; b_ < nBatches; b_++ ){
    ...
    // Look on batch dim
    for(size_t b = 0; b< cfg_.batchSize; b++){
      const auto& cls_data = inputs.clustersX[b_*cfg_.batchSize + b];
      // Loop on clusters
    ...
    // Look on batch dim
    for(size_t b = 0; b< cfg_.batchSize; b++){
      const auto& wind_features = inputs.windowX[b_*cfg_.batchSize + b];
      // Loop on features
    ...
    // Look on batch dim
    for(size_t b = 0; b< cfg_.batchSize; b++){
      const auto& hits_data = inputs.hitsX[b_*cfg_.batchSize + b];
    ...
    for(size_t b = 0; b< cfg_.batchSize; b++, M++){
      *M = float(inputs.clustersX[b_*cfg_.batchSize + b].size());
    }
    ...
    for (size_t b = 0; b < cfg_.batchSize; b++) {
      uint ncls = inputs.clustersX[b_*cfg_.batchSize + b].size();
