...
void DeepSCGraphEvaluation::initTensorFlowGraphAndSession() {
  ...
  for (const auto& modelFile : cfg_.modelFiles) {
    LogDebug("DeepSCGraphEvaluation") << "Loading graph: " << modelFile;
    graphDefs_[imodel] =
        std::unique_ptr<tensorflow::GraphDef>(tensorflow::loadGraphDef(edm::FileInPath(modelFile).fullPath()));
    LogDebug("DeepSCGraphEvaluation") << "Starting TF sessions";
    sessions_[imodel] = tensorflow::createSession(graphDefs_[imodel].get());
    imodel++;
  }
