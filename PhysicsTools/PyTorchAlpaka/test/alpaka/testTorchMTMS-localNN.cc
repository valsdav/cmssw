...
struct SimpleDnnSum : torch::nn::Module {
  torch::Tensor forward(torch::Tensor A, torch::Tensor B) { return A + B; }
};
...
void testTorchFromBufferModelEvalSinglePass(SimpleDnnSum& model,
...
void testTorchFromBufferModelEval::test() {
  ...
  cout << "Will create torch device with type=" << torch_alpaka::kTorchDeviceType
       << " and native handle=" << alpakaDevice.getNativeHandle() << endl;
  torch::Device torchDevice(torch_alpaka::kTorchDeviceType, alpakaDevice.getNativeHandle());
  SimpleDnnSum model;
  ...
  auto a_cpu = alpaka::allocMappedBuf<int32_t, uint32_t, alpaka_common::Vec1D, ALPAKA_ACCELERATOR_NAMESPACE::Platform>(
    alpakaHost, platform, alpaka_common::Vec1D{N});
  auto b_cpu = alpaka::allocMappedBuf<int32_t, uint32_t, alpaka_common::Vec1D, ALPAKA_ACCELERATOR_NAMESPACE::Platform>(
    alpakaHost, platform, alpaka_common::Vec1D{N});
  ...
  for (size_t t = 0; t < threadCount; ++t) {
    threads.emplace_back([&, t] {
      ...
      auto c_cpu = alpaka::allocMappedBuf<int32_t, uint32_t, alpaka_common::Vec1D, ALPAKA_ACCELERATOR_NAMESPACE::Platform>(
        alpakaHost, platform, alpaka_common::Vec1D{N});
