...
ECALOverlapGunProducer::ECALOverlapGunProducer(const ParameterSet& pset) : BaseFlatGunProducer(pset) {
  ParameterSet defpset;
  ParameterSet pgun_params = pset.getParameter<ParameterSet>("PGunParameters");

  fEnFix = pgun_params.getParameter<double>("EnFix");
  ...
  fDeltaZ = pgun_params.getParameter<double>("DeltaZ");
...
ECALOverlapGunProducer::~ECALOverlapGunProducer() {
  // no need to cleanup GenEvent memory - done in HepMCProduct
}

void ECALOverlapGunProducer::produce(Event& e, const EventSetup& es) {
  edm::Service<edm::RandomNumberGenerator> rng;
  CLHEP::HepRandomEngine* engine = &rng->getEngine(e.streamID());

  if (fVerbosity > 0) {
    LogDebug("ECALOverlapGunProducer") << " ECALOverlapGunProducer : Begin New Event Generation" << endl;
  }
  ...
  for (unsigned int ip = 0; ip < particles.size(); ++ip) {
    if (ip >= 1) {
      if (fOverlapping) {
        fR = CLHEP::RandFlat::shoot(engine, tmpR - fDeltaR, tmpR + fDeltaR);
        phi = CLHEP::RandFlat::shoot(engine, tmpPhi - fDeltaPhi, tmpPhi + fDeltaPhi);
        fZ = CLHEP::RandFlat::shoot(engine, tmpZ - fDeltaZ, tmpZ + fDeltaZ);
      } else {
        phi += fDeltaPhi;
      }
  ...
  if (fVerbosity > 0) {
    LogDebug("ECALOverlapGunProducer") << " ECALOverlapGunProducer : Event Generation Done " << endl;
  }
