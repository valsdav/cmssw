...
CloseByParticleMultiGunProducer::CloseByParticleMultiGunProducer(const ParameterSet& pset) : BaseFlatGunProducer(pset) {
...
void CloseByParticleMultiGunProducer::produce(Event& e, const EventSetup& es) {
  ...
  if (fVerbosity > 0) {
    LogDebug("CloseByParticleMultiGunProducer")
        << " CloseByParticleMultiGunProducer : Begin New Event Generation" << endl;
  }
