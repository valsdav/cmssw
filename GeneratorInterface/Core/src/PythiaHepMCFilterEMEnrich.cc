...
PythiaHepMCFilterEMEnrich::PythiaHepMCFilterEMEnrich(const edm::ParameterSet& iConfig)
    : ptSeedThr(iConfig.getParameter<double>("PtSeedThr")),
      ...
      nGoodPairs(iConfig.getParameter<int>("NGoodPairs")),
      nGoodPairsFromMother(iConfig.getParameter<int>("NGoodPairsFromMother")) {}

PythiaHepMCFilterEMEnrich::~PythiaHepMCFilterEMEnrich() {}

bool PythiaHepMCFilterEMEnrich::filter(const HepMC::GenEvent* myGenEvent) {
  bool accepted = false;
  ...
  std::vector<TLorentzVector> candidate, candidateMother, genMother, genPairsFromMother;
