 ...
 *
 * Author: Simon Rothman (MIT), Davide Valsecchi (ETH), Riccardo Salvatico (CERN)
 * Written 2025
 *
 */
...
public:
  explicit DRNCorrectionProducerTDirect(const edm::ParameterSet& iConfig);
...
template <typename T>
DRNCorrectionProducerTDirect<T>::DRNCorrectionProducerTDirect(const edm::ParameterSet& iConfig)
...
template <typename T>
bool DRNCorrectionProducerTDirect<T>::isEB(const T& part) {
  return part.superCluster()->seed()->hitsAndFractions().at(0).first.subdetId() == EcalBarrel;
}

template <typename T>
bool DRNCorrectionProducerTDirect<T>::isEE(const T& part) {
  return part.superCluster()->seed()->hitsAndFractions().at(0).first.subdetId() == EcalEndcap;
}

template <typename T>
bool DRNCorrectionProducerTDirect<T>::skip(const T& part) {
  /*
...
template <typename T>
void DRNCorrectionProducerTDirect<T>::produce(edm::Event& iEvent, const edm::EventSetup& iSetup) {
  /*
...
torch::Device device(torch::kCPU);
  ...
  if (nPart_) {
    auto out_raw = model_.forward(inputs).toTensor();
...
template <typename T>
void DRNCorrectionProducerTDirect<T>::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
